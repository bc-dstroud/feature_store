...
sdf = spark.sql(f"""
                WITH tmp_overfill AS ( -- Needed change table
                SELECT -- Get the current needed for all jobs
                    JOB_ID,
                    OVERFILL AS OLD_OVERFILL,
                    OVERFILL AS NEW_OVERFILL,
                ...
                FROM bc_foreign_mysql.bluecrew._EVENT_OVERFILL_CHANGE
                WHERE OLD_OVERFILL <> NEW_OVERFILL
                ORDER BY JOB_ID, END_DATE),

                max_change as (
                    select job_id, END_DATE, OLD_OVERFILL, NEW_OVERFILL,
                ...
                tmp_overfill2 as (
                select job_id, OLD_OVERFILL, NEW_OVERFILL, end_date
                from max_change
                where id = max_id
                )
                SELECT -- final table with job id and needed count by start/end date
                JOB_ID,
                OLD_OVERFILL AS OVERFILL,
...
table_name = f"feature_store.dev.job_overfill_change"
print(table_name)
